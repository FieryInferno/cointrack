...
const topMonth = computed(
  () => Object.entries(monthly.value).sort((a, b) => b[1] - a[1])[0]
)
...
onMounted(() => {
  ...
  insightText.value!.innerText = `Pengeluaran terbesar terjadi pada bulan ${topMonth.value[0]} dengan total Rp ${topMonth.value[1]}.`
})
</script>
<template>
  ...
  <div class="timeline-box card">
    ...
    <div>
      <div
        class="timeline-item"
        v-for="month in Object.keys(monthly)"
        :key="month"
      >
        <div class="timeline-label">{{ month }}</div>
        <div
          class="timeline-bar"
          :style="{ width: (monthly[month] / maxVal) * 100 + '%' }"
        ></div>
      </div>
    </div>
  </div>
  ...
  <table id="categoryTable">
    ...
    <tr
      ...
      :key="key"
      :class="{ highlight: value === Math.max(...Object.values(categories)) }"
    >
      <td>{{ key }}</td>
